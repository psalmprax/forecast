...
def lambda_handler(event, context):
    # TODO implement
    HOST = "35.156.104.103" #str(os.environ["host"])
    USER = "postgres" #str(os.environ["user"])
    PASS = "secret" #str(os.environ["password"])
    PORT = 5432 #int(os.environ["port"])
    DATABASE = "analytics_v2" #str(os.environ["database"])
    TABLE= "forecasting"#"forecasting"#str(os.environ["table"])
    BENCHMARK = 1000 #"forecasting"#str(os.environ["benchmark"])
    ...
    #print(json_normalize(damage_table))
    ...
    try:
        ...
        data = data.loc[(data['max_trip_mileage']-data['latest_trip_mileage'])>BENCHMARK&data['status']!="Pending"]
    ...
    for index, row2 in data.iterrows():
        ...
        #delete_status = mongodb_connect(host=HOST, database=DATABASE,table=TABLE,command="delete all")
        print("reference_id: ",data_mongodb["reference_id"],"status: ", data_mongodb["status"], \
              "damage_type_id: ", data_mongodb["damage_type_id"],"success: ", data_mongodb["success"], \
              "latest_trip_mileage: ",data_mongodb["latest_trip_mileage"],"component_type_id: " ,data_mongodb["component_type_id"], \
              "vechicle_id: ",data_mongodb["vechicle_id"])
        if COUNT == 10:
            #time.sleep(200)
            break
        COUNT += 1
    ...
    return damage_table#"90 predictions done!!!!!" 
...
def postgres_connect(host=None,user=None,password=None,port=None,database=None, sql_code=None):
    ...
    #postgreSQL_select_Query = "SELECT * FROM public.vehicles"
...
def mongodb_connect(host=None, database=None, table=None, vehcile_id =None, command=None,data=None,search=None):
...
def algorithm(data):
    ...
    #callback_url = None
    #fail_on_error = False
    #ppk = "C:/Users/COMPREDICT/Desktop/POSTGRES_MONGODB/lambda-ppk-pem"
    #passphrase=""
